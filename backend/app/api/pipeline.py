...
async def trigger_ai_prompt(
    ...
    await get_project_for_user(body.project_id, current_user, db)
...
async def start_scoring(
    ...
    """Score all pending images by uncertainty."""
    await get_project_for_user(project_id, current_user, db)
...
async def cancel_task(task_id: str, db: AsyncSession = Depends(get_db),
    ...
    try:
        ...
        job = result.scalar_one_or_none()
        if job:
            # Verify the job belongs to one of the current user's projects
            await get_project_for_user(job.project_id, current_user, db)
            job.status = "failure"
